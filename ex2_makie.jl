...
md"To make plots we'll import the `CairoMakie` package.  (If you're interested, you can click the eyeball to the left of the plot cells to see the plotting code.)"
...
if ready_to_plot && !ismissing(compare_var_calcs(3))
	...
	scatter(N_list, compare_var_calcs.(N_list,true_mean),
			axis = (; title = "Sample mean = $true_mean (Static Plot)",
					xlabel = "Number of observation dates",
					ylabel = "|Δ Estimated Sample Var| (days²)",
				    xscale = log2 )
			)
end
...
if !ismissing(compare_var_calcs(3)) 
	local N_list = [2^n for n in 2:20]
	scatter(N_list, compare_var_calcs.(N_list,true_mean_plt), 
			axis = (;title = "Sample mean = $true_mean_plt",
					xlabel = "Number of observation dates",
					ylabel = "|Δ Estimated Sample Var| (days²)",
				    xscale = log2 )
			)
...
if  !ismissing(compare_var_calcs(3)) && !ismissing(compare_var_calcs_online(3))
	local true_mean = jd_sept1_2021
	local N_list = [2^n for n in 2:20]
	local fig = Figure()
	local panel = Axis(fig[1,1], 
					title = "Sample mean = $true_mean",
					xlabel = "Number of observation dates",
					ylabel = "|Δ Estimated Sample Var| (days²)",
				    xscale = log2,
					)
	
	scatter!(panel,N_list, compare_var_calcs.(N_list,true_mean),  color=:red,label="1-pass minus 2-pass" )
	scatter!(panel,N_list, compare_var_calcs_online.(N_list,true_mean), color=:green,label="online minus 2-pass" )
	axislegend(panel) 
	fig
end
...
manifest_format = "2.0"
project_hash = "8b7133833fb10bcb2be5beb63535f20185b14779"
